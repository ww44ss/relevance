...
condition_text<-function(s){
    ...
    s<-gsub('^ *|(?<= ) | *$', '', s, perl=TRUE)  ## remove mulitple spaces
    return(s)
}
...
##okay, let's make a relevance model
    ...
    ## look at first few characters in each
    ##
    print(substring(product_titles,1, 500))
    print(substring(product_descriptions,1, 500))
    ...
    ##keep nchar sane
    titles_word_Sums<-titles_word_Sums[titles_word_Sums$nchar<11,]
    descriptions_word_Sums<-descriptions_word_Sums[descriptions_word_Sums$nchar<11,]

    ## add a rank index
    titles_word_Sums$rank<-1:dim(titles_word_Sums)[1]
    descriptions_word_Sums$rank<-1:dim(descriptions_word_Sums)[1]

    ## keep only the first 5000 
    titles_word_Sums <- titles_word_Sums[titles_word_Sums$rank<5000,] 
    #titles_word_Sums<-titles_word_Sums[-1,]

    descriptions_word_Sums <- descriptions_word_Sums[descriptions_word_Sums$rank<5000,] 
    #descriptions_word_Sums<-word_Sums[-1,]
    ...
    head(titles_word_Sums, 100)
    head(descriptions_word_Sums, 100)
    ...
    titles_TermDM_1<-TermDocumentMatrix(corpus_titles, control=list(tokenize=OneGramTokenizer))
    titles_TermDM_2<-TermDocumentMatrix(corpus_titles, control=list(tokenize=TwoGramTokenizer))
    titles_TermDM_3<-TermDocumentMatrix(corpus_titles, control=list(tokenize=ThreeGramTokenizer))

    descriptions_TermDM_1<-TermDocumentMatrix(corpus_descriptions, control=list(tokenize=OneGramTokenizer))
    descriptions_TermDM_2<-TermDocumentMatrix(corpus_descriptions, control=list(tokenize=TwoGramTokenizer))
    descriptions_TermDM_3<-TermDocumentMatrix(corpus_descriptions, control=list(tokenize=ThreeGramTokenizer))
    #TermDM <- removeSparseTerms(TermDM, 0.75)

    findFreqTerms(titles_TermDM_1, 10)[1:5]
    findFreqTerms(descriptions_TermDM_1, 10)[1:5]
    findFreqTerms(titles_TermDM_2, 10)[1:5]
    findFreqTerms(descriptions_TermDM_2, 10)[1:5]
    findFreqTerms(titles_TermDM_3, 10)[1:5]
    findFreqTerms(descriptions_TermDM_3, 10)[1:5]
